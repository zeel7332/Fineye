...
export function FavoriteStocksView({ monthLabel, initialData }) {
  ...
  useEffect(() => {
    ...
    const url = FAVORITES_CSV_URL && FAVORITES_CSV_URL.length > 0 ? FAVORITES_CSV_URL : 'Stock_Buy_Feb-26.csv';
    fetchCsv(url)
      .then(data => {
                ...
                paginated.map((row, idx) => (
                  ...
                  <td className="px-4 sm:px-6 py-2 sm:py-2.5 whitespace-nowrap text-xs sm:text-sm max-w-[80px] sm:max-w-none truncate" title={row.sector}>{row.sector || '—'}</td>
                    ...
                    <td className="px-4 sm:px-6 py-2 sm:py-2.5 text-right whitespace-nowrap font-mono text-xs sm:text-sm text-slate-900">
                      {row.net_qty_bought != null ? (row.net_qty_bought / 10000000).toFixed(2) : '—'}
                    </td>
                    <td className="px-4 sm:px-6 py-2 sm:py-2.5 text-right whitespace-nowrap font-mono font-bold text-slate-900 text-xs sm:text-sm">
                      {row.approx_buy_value_cr != null ? row.approx_buy_value_cr.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'}
                    </td>
                  </tr>
                ))
              ) : (
