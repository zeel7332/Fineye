    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-xl font-semibold text-slate-900">{stock?.name}</span>
      ...
      <div className="flex gap-3">
        <input
          ...
          className="w-full max-w-md px-3 py-2 rounded-md border border-slate-300 bg-white text-slate-800 focus:outline-none"
          value={search}
              ...
              {rows
                ...
                .map((fund, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/50">
                    <td className="px-6 py-3 font-medium text-slate-800">{fund.fund_name}</td>
                    <td className="px-6 py-3">{fund.classification || '—'}</td>
                    <td className="px-6 py-3">{fund.month || '—'}</td>
                    <td className="px-6 py-3 text-right">{typeof fund.percent_aum === 'number' ? `${fund.percent_aum}%` : '—'}</td>
                  </tr>
                ))}
            </tbody>
          </table>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        {showHero && (
          <div className="relative text-center py-4">
            <h2 className="text-2xl font-semibold text-slate-900 mb-2">Mutual Fund Insights</h2>
            <p className="text-base text-slate-600 max-w-2xl mx-auto">Clear, simple views into how mutual funds are investing.</p>
            <button
              onClick={() => {
                setShowHero(false);
                if (typeof window !== 'undefined' && window.localStorage) {
              ...
              className="absolute top-2 right-2 inline-flex items-center justify-center w-8 h-8 rounded-md border border-slate-300 bg-white text-slate-600 hover:bg-slate-50"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="min-h-[400px]">
          {activeView === 'dashboard' && !fundsPage && activeTab === 'smart-money' && (
            loading ? (
              <div className="flex items-center justify-center py-20 text-slate-500">Loading data…</div>
            ) : error ? (
          ...
          {activeView === 'about' && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
              <h3 className="text-xl font-semibold text-slate-900">About FinEye</h3>
              ...
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">Purpose</h4>
                <p className="text-slate-700">Learning and research. We present insights to make disclosures easier to understand.</p>
              </div>
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">Data Sources</h4>
                <p className="text-slate-700">Public mutual fund disclosures compiled into a structured dataset.</p>
                <p className="text-xs text-slate-500">Data freshness depends on the latest available disclosures.</p>
              </div>
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">What FinEye does</h4>
                <p className="text-slate-700">Shows which stocks mutual funds are buying or holding.</p>
              </div>
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">What FinEye does NOT do</h4>
                <p className="text-slate-700">No stock recommendations, no tips, and no guarantees.</p>
              </div>
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">Disclaimer</h4>
                <p className="text-slate-700">For educational and informational purposes only. You are responsible for your decisions.</p>
                <p className="text-xs text-slate-500">Always do your own research.</p>
              </div>
              <div className="space-y-2">
                <h4 className="font-semibold text-slate-800">North Star</h4>
                <p className="text-slate-700">FinEye helps you observe smart money — not follow it blindly.</p>
              </div>
            </div>
          )}
        </div>
